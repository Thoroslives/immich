...
class ServerThemeDto {
  ...
  static ServerThemeDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'customCss'), 'Required key "ServerThemeDto[customCss]" is missing from JSON.');
        assert(json[r'customCss'] != null, 'Required key "ServerThemeDto[customCss]" has a null value in JSON.');
        return true;
      }());
