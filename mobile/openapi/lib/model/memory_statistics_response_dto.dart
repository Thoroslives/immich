...
class MemoryStatisticsResponseDto {
  ...
  static MemoryStatisticsResponseDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'total'), 'Required key "MemoryStatisticsResponseDto[total]" is missing from JSON.');
        assert(json[r'total'] != null, 'Required key "MemoryStatisticsResponseDto[total]" has a null value in JSON.');
        return true;
      }());
