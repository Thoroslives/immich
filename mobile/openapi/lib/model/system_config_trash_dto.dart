...
class SystemConfigTrashDto {
  ...
  static SystemConfigTrashDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'days'), 'Required key "SystemConfigTrashDto[days]" is missing from JSON.');
        assert(json[r'days'] != null, 'Required key "SystemConfigTrashDto[days]" has a null value in JSON.');
        assert(json.containsKey(r'enabled'), 'Required key "SystemConfigTrashDto[enabled]" is missing from JSON.');
        assert(json[r'enabled'] != null, 'Required key "SystemConfigTrashDto[enabled]" has a null value in JSON.');
        return true;
      }());
